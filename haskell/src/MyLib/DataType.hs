...
treeInsert x (TreeNode a left right) = case compare x a of
  EQ -> TreeNode x left right
  LT -> TreeNode a (treeInsert x left) right
  GT -> TreeNode a left (treeInsert x right)

treeElem :: (Ord a) => a -> Tree a -> Bool
treeElem _ EmptyTree = False
treeElem x (TreeNode a l r) = case compare x a of
  LT -> treeElem x l
  EQ -> True
  GT -> treeElem x r
