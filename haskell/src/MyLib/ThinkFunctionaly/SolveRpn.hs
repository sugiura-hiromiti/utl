module MyLib.ThinkFunctionaly.SolveRpn where

import Control.Monad (foldM)
import Text.Read (readMaybe)

-- abc
solveRpn :: (Num a, Read a, Fractional a) => String -> Either RpnSolverError a
solveRpn expr = parseTokens (words expr) >>= eval

parseTokens :: (Num a, Read a) => [String] -> Either RpnSolverError [Token a]
parseTokens = traverse parseToken

data RpnSolverError = UnparsableToken | InvalidSyntax Invalidity
  deriving (Show, Eq)
...
parseToken :: (Num a, Read a) => String -> Either RpnSolverError (Token a)
...
eval :: (Num a, Fractional a) => [Token a] -> Either RpnSolverError a
eval ts = do
...
evalNextToken :: (Num a, Fractional a) => [a] -> Token a -> Either RpnSolverError [a]
